  ...
  return (
    <div className="border border-[#1F1F2E] rounded-xl overflow-hidden">
      <button
        ...
        className="w-full flex items-center gap-3 p-3.5 text-left hover:bg-[#1A1A2E] transition-colors"
      >
        <span className="text-lg flex-shrink-0">{section.emoji}</span>
        <span className="flex-1 font-bold text-sm text-[#E5E7EB]">{section.title}</span>
        {open ? (
      ...
      {open && (
        <div className="px-3.5 pb-3.5 animate-fade-in">
          <p className="text-sm text-[#9CA3AF] leading-relaxed mb-2">{section.content}</p>
          {section.tips && section.tips.length > 0 && (
            <div className="bg-[#0A0A0F] rounded-lg p-3 space-y-1.5">
              {section.tips.map((tip, i) => (
                <div key={i} className="flex items-start gap-2 text-xs text-[#9CA3AF] leading-relaxed">
    ...
    <div className="dashboard-card overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-wider">{week.weeks}</p>
          <h3 className="font-bold text-[#E5E7EB] text-sm">{week.title}</h3>
          <p className="text-[11px] text-[#9CA3AF] mt-0.5">{week.sections.length}개 주제</p>
        </div>
        <ChevronDown
          size={18}
          className={`text-[#9CA3AF] transition-transform flex-shrink-0 ${expanded ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {expanded && (
        <div className="px-4 pb-4 animate-fade-in space-y-2">
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#1A1A2E] text-[#9CA3AF] rounded-xl text-xs font-bold hover:bg-[#1F1F2E] transition-colors no-underline mt-3"
          >
...
export default function LecturesPage() {
  return (
    <div className="animate-fade-in max-w-[520px] mx-auto px-6 pt-8 pb-24">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-[#E5E7EB] tracking-tight">교안 자료</h1>
        <p className="text-[#9CA3AF] font-medium text-sm mt-1">주차별 강의 내용을 읽고 복습하세요!</p>
      </div>
      ...
      <div className="mt-6 dashboard-card p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-xs font-bold text-[#E5E7EB] mb-0.5">학습 팁</p>
